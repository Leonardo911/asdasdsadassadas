/*
Tabla de registro de productos
*/

class Producto {
    constructor() {
        this.id = -1;
        this.clave = '';
        this.nombre = '';
        this.descripcion = '';
        this.partidaDetalleId = -1;
        this.sku = '';
        this.unidad = '';
        this.marca = '';
        this.modelo = '';
        this.proveedorIdUltimo = -1;
        this.precioUltimo = parseFloat(0);
        this.categoriaId = -1
        this.partidaDetalleNombre = '';
        this.proveedorNombreUltimo = '';
        this.categoriaNombre = '';
    }

    copyValues(item) {
        this.id = item.id;
        this.clave = item.clave;
        this.nombre = item.nombre;
        this.descripcion = item.descripcion;
        this.partidaDetalleId = item.partidaDetalleId;
        this.sku = item.sku;
        this.unidad = item.unidad;
        this.marca = item.marca;
        this.modelo = item.modelo;
        this.proveedorIdUltimo = item.proveedorIdUltimo;
        this.precioUltimo = item.precioUltimo;
        this.categoriaId = item.categoriaId;
        this.partidaDetalleNombre = item.partidaDetalleNombre;
        this.proveedorNombreUltimo = item.proveedorNombreUltimo;
        this.categoriaNombre = item.categoriaNombre;

        return this;
    }

    reset() {
        this.id = -1;
        this.clave = '';
        this.nombre = '';
        this.descripcion = '';
        this.partidaDetalleId = -1;
        this.sku = '';
        this.unidad = '';
        this.marca = '';
        this.modelo = '';
        this.proveedorIdUltimo = -1;
        this.precioUltimo = parseFloat(0);
        this.categoriaId = -1
        this.partidaDetalleNombre = '';
        this.proveedorNombreUltimo = '';
        this.categoriaNombre = '';

        $('#categoriaId').val('');
    }

    updateValues(original, item) {
        original.id = item.id;
        original.clave = item.clave;
        original.nombre = item.nombre;
        original.descripcion = item.descripcion;
        original.partidaDetalleId = item.partidaDetalleId;
        original.sku = item.sku;
        original.unidad = item.unidad;
        original.marca = item.marca;
        original.modelo = item.modelo;
        original.proveedorIdUltimo = item.proveedorIdUltimo;
        original.precioUltimo = item.precioUltimo;
        original.categoriaId = item.categoriaId;
        original.partidaDetalleNombre = item.partidaDetalleNombre;
        original.proveedorNombreUltimo = item.proveedorNombreUltimo;
        original.categoriaNombre = item.categoriaNombre;

        return original;
    }
}

class Categoria {
    constructor() {
        this.id = -1;
        this.nombre = '';
    }

    copyValues(item) {
        this.id = item.id;
        this.nombre = item.nombre
    }
}

var model;
var grid;

$(document).ready(function () {
    model = new Vue({
        el: '#appVue',
        data: {
            categorias: [],
            productos: [],
            current: new Producto()
        },
        created() {
            var me = this;
            showSpinner();

            $.ajax({
                type: 'POST',
                dataType: 'json',
                url: '/Compras/ProductosGet',
                success: function (result, status, xhr) {

                    hideSpinner();
                    if (result.success == false) {
                        alertify.error(result.error);
                        return;
                    }

                    $.each(result.productos, function () {
                        me.productos.push(new Producto().copyValues(this));
                    })

                    var s = '<option placeholder="" value="">Seleccione un registro</option>';
                    $.each(result.categorias, function () {
                        me.categorias.push(new Categoria().copyValues(this));
                        s += '<option value="' + this.id + '">' + this.nombre + '</option>';
                    });
                    $("#categoriaId").html(s);

                    setTimeout(function () {
                        const selectChoices = new CWhoices('#categoriaId',
                            {
                                shouldSort: false,
                                placeholder: true,
                                searchPlaceholderValue: false,
                                itemSelectText: ''
                            });
                    }, 500);

                    $("#categoriaId").change(function () {
                        me.current.categoriaId = $(this).find(":selected").val();
                        me.current.categoriaNombre = $(this).find(":selected").text();
                    });

                    grid = new gridjs.Grid({
                        columns:
                            [
                                {
                                    id: 'id',
                                    hidden: true
                                },
                                {
                                    id: 'clave',
                                    name: 'Clave'
                                },
                                {
                                    id: 'nombre',
                                    name: 'Nombre'
                                },
                                {
                                    id: 'descripcion',
                                    name: 'Descripcion'
                                },
                                {
                                    id: 'partidaDetalleNombre',
                                    name: 'Partida'
                                },
                                {
                                    name: "Acciones",
                                    sort: {
                                        enabled: false
                                    },
                                    formatter: (cell, row) => {
                                        return gridjs.html('<div class="d-flex gap-3"><a href="javascript:void(0);" onclick="editaRegistro(' + row.cells[0].data + ');" data-bs-toggle="modal" data-bs-target=".modalEdit" data-bs-toggle="tooltip" data-bs-placement="top" title="Editar" class="text-success"><i class="mdi mdi-pencil font-size-18"></i></a><a href="javascript:void(0);" onclick="eliminaRegistro(' + row.cells[0].data + ');" data-bs-toggle="tooltip" data-bs-placement="top" title="Delete" class="text-danger"><i class="mdi mdi-delete font-size-18"></i></a></div>');
                                    }
                                }
                            ],
                        pagination: {
                            limit: 8
                        },
                        sort: true,
                        search: true,
                        data: me.productos,
                        language: {
                            'search': {
                                'placeholder': '🔍 Buscar...'
                            },
                            'pagination': {
                                'previous': 'Anterior',
                                'next': 'Siguiente',
                                'showing': 'Mostrando',
                                'results': () => 'Registros',
                                'to': 'a',
                                'of': 'de'
                            }
                        }
                    }).render(document.getElementById("table-catalog"));

                    $('#btnAdd').show();
                },
                error: function (error) {
                    hideSpinner();
                }
            });
        },
        methods: {
            guardaRegistro: function () {
                var me = this;

                if (!validaRegistro()) {
                    return;
                }

                showSpinner();

                $.ajax({
                    type: 'POST',
                    dataType: 'json',
                    data: model.current,
                    url: '/Compras/ProductoSave',
                    success: function (result, status, xhr) {

                        hideSpinner();

                        if (result.success == false) {
                            alertify.error(result.error);
                            return;
                        }

                        if (me.current.id > 0) {
                            me.actualizaRegistro(result);
                        } else {
                            me.agregaRegistro(result);
                        }

                        grid.updateConfig({
                            data: me.productos
                        }).forceRender();

                        $('#modalEdit').modal('hide');

                        alertify.success('Guardado exitoso');
                    },
                    error: function (error) {
                        hideSpinner();
                    }
                });
            },
            actualizaRegistro: function (result) {
                var producto = this.productos.find(obj => obj.id == result.model.id);
                producto.updateValues(producto, result.model);
            },
            agregaRegistro: function (result) {
                this.productos.push(new Producto().copyValues(result.model));
            },
            eliminaRegistro: function (id) {
                var me = this;
                showSpinner();

                $.ajax({
                    type: 'POST',
                    dataType: 'json',
                    data: { id: id },
                    url: '/Compras/ProductoDelete',
                    success: function (result, status, xhr) {

                        hideSpinner();

                        if (result.success == false) {
                            alertify.error(result.error);
                            return;
                        }

                        let index = me.productos.map((item) => item.id).indexOf(id);
                        if (index >= 0) {
                            me.productos.splice(index, 1);

                            grid.updateConfig({
                                data: me.productos
                            }).forceRender();
                        }
                    },
                    error: function (error) {
                        hideSpinner();
                    }
                });
            }
        }
    });
})

function validaRegistro() {
    if (model.current.clave && model.current.clave.length > 0 &&
        model.current.nombre && model.current.nombre.length > 0 &&
        model.current.descripcion && model.current.descripcion.length > 0)
        return true;

    alertify.warning('Debe indicar todos los campos del registro');
    return false;
}

function editaRegistro(id) {
    if (id < 0) {
        return;
    }

    var producto = model.productos.find(obj => obj.id == id);
    if (producto) {
        model.current = new Producto().updateValues(model.current, producto);
        $('#categoriaId').val(producto.categoriaId);
    }
}

function eliminaRegistro(id) {
    if (id <= 0) {
        return;
    }

    Swal.fire({
        title: "¿Confirma que desea eliminar el producto seleccionado?",
        text: "El proceso no es reversible!",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#51d28c",
        cancelButtonColor: "#f34e4e",
        confirmButtonText: "Eliminar",
        cancelButtonText: "Cancelar"
    }).then(function (result) {
        if (result.value) {
            model.eliminaRegistro(id);
        }
    });
}